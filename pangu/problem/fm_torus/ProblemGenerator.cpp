...
void ProblemGenerator(parthenon::MeshBlock *pmb, parthenon::ParameterInput *pin) {
    ...
    const Real kappa = pin->GetOrAddReal("torus", "kappa", 1e-3);
    const Real rho_floor = pin->GetOrAddReal("torus", "rho_floor", 1e-6);
    const Real p_floor = pin->GetOrAddReal("torus", "p_floor", 1e-8);
    const Real b0 = pin->GetOrAddReal("torus", "b0", 0.0);
